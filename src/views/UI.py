...
# ↑se deben mbuscar los libros del usuario para mostrarlos
    ...
    def books_prestados_for_client(self):
        books_prestados = []
        prestamos_del_usuario = self.cliente.prestamos
        if prestamos_del_usuario != None:
    ...
    def addbookconfirm(self):
        ...
        estante = self.control.get_estante_by_genero(shelveName)
        numBooks= len(estante.libros)
...
# _______________busqueda de libros ________________________________________
    ...
    def lending(self, book):
        ...
        self.control.new_prestamo(fecha= datetime.now(), id_cliente = user_a_prestar.codigo_Usuario, nombre = libro[0].nombre)
        self.showbook(libro[0].nombre, libro[0].autores[0], libro[0].genero, libro[0].estado)
    ...
    def returnBook(self, book):
        ...
        padre.deleteLater()
        print('el cliente tienes estos prestamos', self.cliente.prestamos.codigo, ' y el libro esste al momento de devolver', libro[0].estado.codigo)
        print('regresando libro:', libro[0].estado.libro.nombre, 'prestado por', libro[0].estado.cliente.nombre)
        libro[0].estado.devuelto = True
        for prestamo in self.cliente.prestamos:
            if prestamo.codigo == libro[0].estado.codigo:
...
class ProfileWindow(QMainWindow):     # ventana perfil de usuario
    ...
    def otherLines(self):
        if self.control.verificar_cliente(self.userName, 'haahah', 2):
            dic = {'1': 'Nombre Completo', '2': f'{self.control.get_cliente(id = 3, name = self.userName).contrasena}', '3': 'Correo electrónico'}
        else:
            dic = {'1': 'Nombre Completo', '2': f'{self.control.get_admin_by_name( name = self.userName).contrasena}', '3': 'Correo electrónico'}
        widget = QWidget(self)
